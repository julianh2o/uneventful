...
describe('Events API Endpoints', () => {
	...
	describe('POST /api/events', () => {
		...
		it('should associate event with authenticated user', async () => {
			...
			const getResponse = await request(app).get(`/api/events/${eventId}`).set('Authorization', `Bearer ${user1Token}`);
	...
	describe('GET /api/events', () => {
		...
		it('should return only events for authenticated user', async () => {
			const response = await request(app).get('/api/events').set('Authorization', `Bearer ${user1Token}`);
	...
	describe('GET /api/events/:id', () => {
		...
		it('should return event for owner', async () => {
			const response = await request(app).get(`/api/events/${eventId}`).set('Authorization', `Bearer ${user1Token}`);
		...
		it('should return 403 for non-owner', async () => {
			const response = await request(app).get(`/api/events/${eventId}`).set('Authorization', `Bearer ${user2Token}`);
